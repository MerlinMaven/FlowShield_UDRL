#!/usr/bin/env python3
"""
Train Flow Matching Shield - Entraîne le shield Flow Matching séparément.

Le modèle est sauvegardé dans: results/<env>/models/flow_shield.pt
Les courbes dans: results/<env>/figures/flow_training.png
...
Usage:
    python scripts/train_flow.py --data data/lunarlander_xxx.npz --epochs 100
    python scripts/train_flow.py --data data/lunarlander_xxx.npz --epochs 100 --patience 20
"""
...
from models import (
    FlowMatchingShield, load_data, ensure_results_dirs, save_model,
    set_seed, EarlyStopping, CosineScheduler, GradientClipper
)


def train_flow_shield(
    ...
    """Train the Flow Matching Shield with early stopping and calibration."""
    ...
    # Calibrate OOD threshold after training
    if verbose:
...
def main():
    parser = argparse.ArgumentParser(description="Train Flow Matching Shield")
    ...
    print("=" * 60)
    print("TRAINING FLOW MATCHING SHIELD")
    print("=" * 60)
    ...
    # Train
    train_losses, val_losses = train_flow_shield(
        shield,
        data['states'],
        data['commands'],
        epochs=args.epochs,
    ...
    save_model(shield, env_name, "flow_shield")
    
    # Plot
    plot_path = results_dir / "figures" / "flow_training.png"
    plot_training_curve(train_losses, val_losses, plot_path)
    
    print("\n" + "=" * 60)
    print("TRAINING COMPLETE")
    print("=" * 60)
    print(f"Model saved: results/{env_name}/models/flow_shield.pt")
    print(f"Figure saved: results/{env_name}/figures/flow_training.png")
    print("=" * 60)
